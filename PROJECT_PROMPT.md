...
This is a CLI tool that allows read-only access to Google Docs, Google Sheets, Google Slides, and uses Google OAuth API tokens.
...
5. Implement output in JSON or Markdown, `-o json` or `-o markdown` in CLI options. This replaces or works in addition to `docx` and `csv`. Tabular spreadsheet data should use `csv` instaed of `markdown`
